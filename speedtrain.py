...
# train calculation
def realtime_speed(video_src):
    # initialize
    cascade_src = 'E:/integration/Speed_Detection_CV/cascade/classifier/cascade.xml'
    #video_src = 'D:/sem_7/integration/sakura.mp4'
    #line a
    ax1=70
    ay=90
    ax2=230
    ...
    print('time \t\t speed of train \t result')
        ...
        for (x,y,w,h) in cars:
            ...
            while int(ay) <= int((y+y+h)/2):
                if int(by) <= int((y+y+h)/2)&int(by+10) >= int((y+y+h)/2):
                    ...
                    if (Speed*10)>40:
                    # time,speed
                        print(now.strftime("%H:%M"), "\t\t", (Speed*10), "\t\t", "Reduce the speed of the train")
                        break
                    else:
                        print(now.strftime("%H:%M"), "\t\t", (Speed*10), "\t\t", "Speed of the train is nominal")
                        break
